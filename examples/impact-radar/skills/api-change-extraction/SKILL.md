...
1. Ensure old and new snapshots exist under
   `$OMNIGENT_AGENT_BUNDLE_DIR/data/pricing/` as
...
2. Run the extractor into the run directory:
   ```
   python3 "$OMNIGENT_AGENT_BUNDLE_DIR/scripts/extract_change_cards.py" \
     --pricing-dir "$OMNIGENT_AGENT_BUNDLE_DIR/data/pricing" \
     --out runs/<run_id>/change_cards.json
   ```
